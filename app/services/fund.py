...
def get_fund_summary(request: FundRequest) -> dict[str, any]:
    ...
    volatility = stk["return"].std() * 100 * (n_days ** .5)
    volatility_annualized = stk["return"].std() * 100 * (252 ** .5)
    dividends = stk["dividends"].sum()
    dividend_yield = dividends / value * 100
    sharpe_ratio = (total_return_annualized - total_return_rf_annualized) / volatility_annualized
    tracking_error = df_wide["return_active"].std() * 100 * (n_days ** .5)
    tracking_error_annualized = df_wide['return_active'].std() * 100 * (252 ** .5)
